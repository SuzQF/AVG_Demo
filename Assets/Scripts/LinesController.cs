...
public class LinesController : MonoBehaviour {
	...
	//台词显示所用UI
	[SerializeField]public Text linesTxt;
	...
	private void TxtPositionFollow() {
		...
		if (isRect) {
			//根据当前说话角色调整台词UI显示偏移量
			linesTxt.rectTransform.anchoredPosition = new Vector2(pos.x, pos.y+ 125);
		}
	}
	...
	private void InitTable(string[] sides) {
		...
		int half = sides.Length / 2;

		//根据side数量设置side按钮排布
		float distance = 300.0f / half;
		for (int i = 0; i < half; i++) {
			...
			//设置按钮名字以便开启对应脚本
			SideChooseTable.GetChild(i).name = sides[i];

			//手动给按钮添加点击事件，使其被点击时，将side改为自己对应的编号
			SideChooseTable.GetChild(i).GetComponent<Button>().onClick.AddListener(() => { side = i; });

			//设置按钮内容
			SideChooseTable.GetChild(i).GetChild(0).GetComponent<Text>().text = sides[i+half];
	...
	public void SideLineChoosed() {
		//关闭选择协程
		instance.StopCoroutine("ChooseSideLineCoroutine");

		//隐藏UI
		instance.SideChooseTable.gameObject.SetActive(false);

		//开启对应的脚本
		MainStoryScript.instance.ChooseStoryChapter(instance.SideChooseTable.GetChild(side).name);
