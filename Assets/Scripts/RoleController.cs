...
public class RoleController : MonoBehaviour {
	...
	private void Patrol() {
		if (GM.JudgeReachGoalPosition(transform.position, PatrolPosition1)) {
			goalPosition = PatrolPosition2;
		}
		if (GM.JudgeReachGoalPosition(transform.position, PatrolPosition2)) {
			goalPosition = PatrolPosition1;
		}
		moveSpeed = 1f;
		Move();
