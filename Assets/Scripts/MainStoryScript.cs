...
public class MainStoryScript : PerformanceManagerBase {
	...
	public void ChooseStoryChapter(string chapterTopic) {
		if (Convert.ToInt32(chapterTopic.Split('_')[1])<chapterCount) {
			return;
		}
		ChapterStart("Story");
	...
	/// </summary>
	/// <returns></returns>
	public IEnumerator FirstToVillage_0() {
		while (!isChapterEnd) {
			switch (currentPerformance) {
				case 0:
					...
					//LinesController.instance.linesTxt.gameObject.SetActive(true);
	...
	/// </summary>
	/// <returns></returns>
	public IEnumerator TalkWithBearded_1() {
		while (!isChapterEnd) {
			switch (currentPerformance) {
				case 0:
					GM.Log("Arvin向右移动");
	...
	/// </summary>
	/// <returns></returns>
	public IEnumerator MeetWithAkauri_2() {
		while (!isChapterEnd) {
			switch (currentPerformance) {
				case 0:
					cameraController = FindObjectOfType<CameraController>();
					...
					SoundController.instance.PlaySound(GameObject.Find("Door2").transform, "Door2");

					yield return StartCoroutine(cameraController.CameraShrink(4, 1));
					...
					SoundController.instance.PlaySound(RoleManager.instance.FindRoleController(RoleManager.Akauri).transform, "WomanBreath");

					yield return StartCoroutine(RoleManager.instance.FindRoleController(RoleManager.Akauri).RoleMove(new Vector3(9, -0.819f, 0),2));
					break;
				case 2:
				...
				case 3:
					...
					LinesController.instance.ChooseSideLine("GoWithAkauri_3", "BackToVillage_3","和阿卡瑞丽一起去","回村庄等待");
	...
	/// </summary>
	/// <returns></returns>
	public IEnumerator GoWithAkauri_3() {
		while (!isChapterEnd) {
			switch (currentPerformance) {
				case 0:
	...
	/// </summary>
	/// <returns></returns>
	public IEnumerator BackToVillage_3() {
		while (!isChapterEnd) {
			switch (currentPerformance) {
				case 0:
					...
					StartCoroutine(RoleManager.instance.FindRoleController(RoleManager.Arvin).RoleMove(new Vector3(0f, -0.714f, 0)));

					StartCoroutine(ScenesManager.instance.ChangeScene(Color.black, ScenesManager.ENDSCENE2));
		...
		while (!isChapterEnd) {
			switch (currentPerformance) {
				...
				case 7:
					...
					break;
				case 8:
					StartCoroutine(ScenesManager.instance.ChangeScene(Color.black, "MainMenu"));
					ChapterEnd();
					yield break;
				default:
					break;
